...
static void convertMp4ToMov(string pathMp4, string pathMov)
{
    string systemCall = "ffmpeg -i " + pathMp4 + " -f mov " + pathMov +" -y";
...
void generateAllPunks(const vector<string> &layerDir, const vector<vector<string>> &singleLayer, const int numbOfPunkToGen, const int scale, const string &outputExtension)
{
    ...
    while(currGen<=numbOfPunkToGen){
        ...
        if(outputExtension==".mov")
            convertMp4ToMov("../tmp/" + to_string((int)layerDir.size()-1) + ".mp4", "../output/images/" + to_string(currGen) + ".mov");
        ++currGen;
    }
    fin.close();
    return;
